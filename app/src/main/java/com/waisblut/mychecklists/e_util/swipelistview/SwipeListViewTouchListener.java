package com.waisblut.mychecklists.e_util.swipelistview;
...
import com.waisblut.mychecklists.e_util.swipelistview.animation.Animator;
import com.waisblut.mychecklists.e_util.swipelistview.animation.AnimatorListenerAdapter;
import com.waisblut.mychecklists.e_util.swipelistview.animation.ValueAnimator;
import com.waisblut.mychecklists.e_util.swipelistview.view.ViewHelper;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

import static com.waisblut.mychecklists.e_util.swipelistview.view.ViewHelper.setAlpha;
import static com.waisblut.mychecklists.e_util.swipelistview.view.ViewHelper.setTranslationX;
import static com.waisblut.mychecklists.e_util.swipelistview.view.ViewPropertyAnimator.animate;
        ...
        implements View.OnTouchListener
{
                                      ...
                                      int swipeBackView)
    {
        ...
        configShortAnimationTime = swipeListView.getContext()
                                                .getResources()
    ...
    public void resetItems()
    {
        ...
        {
            int count = swipeListView.getAdapter()
                                     .getCount();
    ...
    {
        final View child = swipeListView.getChildAt(
    ...
    protected void closeAnimate(int position)
    {
        if (swipeListView != null)
        {
            ...
            final View childContainer = swipeListView.getChildAt(
                    position - firstVisibleChildPosition);
     ...
     * @param position  Position of list
     */
    private void generateAnimate(final View view,
                                 final boolean swap,
                                 final boolean swapRight,
                                 final int position)
    {
        if (SwipeListView.DEBUG)
        {
            Log.d(SwipeListView.TAG,
     ...
     * @param position  Position of list
     */
    private void generateDismissAnimate(final View view,
                                        final boolean swap,
                                        final boolean swapRight,
                                        final int position)
    {
        ...
        if (opened.get(position))
        {
            if (!swap)
            {
                moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset) : (int) (
    ...
    public boolean onTouch(View view, MotionEvent motionEvent)
    {
        ...
        case MotionEvent.ACTION_DOWN:
        {
            ...
            for (int i = 0; i < childCount; i++)
            {
                ...
                boolean allowSwipe = swipeListView.getAdapter()
                                                  .isEnabled(childPosition) &&
                                     swipeListView.getAdapter()
                                                  .getItemViewType(childPosition) >= 0;
        ...
        case MotionEvent.ACTION_MOVE:
        {
                ...
                velocityY < velocityX)
            {
                ...
                {
                    Log.d(SwipeListView.TAG,
                          "deltaX: " + deltaX + " - swipingRight: " + swipingRight);
                }
                if (opened.get(downPosition))
                {
                ...
                cancelEvent.setAction(MotionEvent.ACTION_CANCEL | (MotionEventCompat.getActionIndex(
                        motionEvent) << MotionEventCompat.ACTION_POINTER_INDEX_SHIFT));
            ...
            if (swiping && downPosition != ListView.INVALID_POSITION)
            {
                if (opened.get(downPosition))
                {
                    deltaX += openedRight.get(downPosition) ?
                            viewWidth - rightOffset : -viewWidth + leftOffset;
                }
                move(deltaX);
                return true;
            }
    ...
    public void move(float deltaX)
    {
        ...
        if (opened.get(downPosition))
        {
            posX += openedRight.get(downPosition) ?
                    -viewWidth + rightOffset : viewWidth - leftOffset;
        }
